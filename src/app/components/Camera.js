...
function Camera() {
  ...
  function capturePhoto() {
    ...
    if (!video || !canvas || !video.videoWidth) {
      setErrorMessage("Kamera belum siap coba ditunggu")
    }
  ...
  function onSubmit(e) {
    if (!photoDataUrl) {
      e.preventDefault();
      setErrorMessage("Belum ada foto")
      return
    }
  ...
  const htmlToRender = cleanUpHtml(typeHtml || (isTyping ? "" : responseHtml) || "")
    ...
    <div>
      <div className='relative w-full rounded-2xl overflow-hidden bg-black'>
        <Webcam
          ...
          className={`w-full ${isPortrait ? "aspect-[9/16]" : "aspect[16/9]"} object-cover`}
          mirrored
        ...
        {photoDataUrl && (
          <img src={photoDataUrl} alt='capture' className='absolute inset-0  w-full h-full 
          object-cover'/>
        )}
        ...
        <div className='absolute bottom-2 left-1/2 -translate-1/2 flex items-center gap-3'>
          {!photoDataUrl ? (
            <button onClick={capturePhoto} className='flex items-center justify-center shadow w-14 h-14 rounded-full bg-white text-gray-900' title='Ambil foto'>
              <FiCamera className='w-6 h-6' />
            </button>
          ) : (
            <button onClick={retake} className='flex items-center justify-center shadow w-14 h-14 rounded-full bg-white text-gray-900' title='Retake foto'>
